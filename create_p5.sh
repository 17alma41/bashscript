...
# Crea la carpeta principal p5 si no existe y navega a ella
mkdir -p p5
cd p5
...
if [ -z "$last_num" ]; then
  new_num=0 # new_num es la variable que usamospara indicarle el valor a los nuevos sketches
else
  new_num=$((last_num + 1))
fi
...
# Crea el archivo JavaScript correspondiente para el sketch
echo 'function setup() {
